...
`fisk2010jurisdiction`（**書誌はCrossrefの出版社寄託と完全一致**＝著者 Catherine L. Fisk・_English Language Notes_ 48(2):15--25・2010-09-01。抄録が寄託されておらず本文は有料のため内容は未確認だが、本稿が帰している主張「管轄こそが著作者性を決める変数である」は**論文タイトルそのものの主張**であり、同一著者の2012年論文を全文で読んで同じ主張を確認しているため低リスク）、`fourpillars2025anime`（HTTP 429が継続）、`kimura2020toei`（日本語単行本。書誌情報は OpenBD で一致、内容は未確認＝**図書館で要確認**）、`menger1999artistic` → **解決済み**。Crossrefに寄託された抄録を読んだところ、本稿が帰していた「評判が存在しない資格の代わりを務める」は抄録のどこにも無く、抄録が支持するのは「多くの芸術世界では初期訓練が不完全な選別装置である」。本文を抄録が支持する範囲に書き直した（論旨は変わらない）。
...
**2026-09-13 オーナー決定: 復活させない。** 論旨に必須でなく、投稿を待たせる理由がない。将来この論点を強化したくなった場合の候補は Pruvost-Delaspre, *Mechademia: Second Arc* 16(2), 2023（東映動画における「演出」クレジットの発明。Project MUSEで有料）だが、本稿では扱わない。
...
本文から引用されなくなったため `references.bib` から削除した。書誌情報自体は確認済みなので、復活させる場合はこれをそのまま戻せばよい。`kimura2020toei` は**復活させない決定**（G-6）。記録のためだけに残す。
