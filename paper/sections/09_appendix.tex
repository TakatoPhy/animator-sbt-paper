...
\Cref{tab:test_results} lists this v1 suite. A further 35 tests accompany the
...
canonicalization), 10 for the disclosure commitment of \cref{subsec:privacy}
(no identifier published, a presented disclosure verifies, a substituted name or
salt fails, altered public attributes fail, and distinct salts leave two
certificates for one person unlinkable), and 1 local end-to-end issuance test
(build metadata and canonical hash $\rightarrow$ mint $\rightarrow$ studio co-sign
$\rightarrow$ verify), for \textbf{70 passing tests} in total.

\begin{table}[ht]
\centering
\caption{Unit test suite for the deployed v1 \texttt{AnimatorSBT} contract
(35 tests, all passing). A further 35 tests cover the v2 co-signing reference
